...
/* ── REAL PROJECT URLS ──────────────────────────────────────────────────
   Each case study has its own page at projects/<slug>/ (generated by
   tools/build-pages.mjs), so it can be shared with its own link preview
   and indexed on its own. Those pages are copies of index.html with
   <html data-project="slug"> and a <base href="../../">, so every relative
   path in content.js still resolves from the site root.

   The old hash routes (#/project/slug) still work, so links already sent
   out don't break, and they're what a local file:// preview uses, since a
   folder URL doesn't open its index.html there. */
const PAGE_PROJECT = document.documentElement.dataset.project || null;
const ROOT_URL     = new URL('./', document.baseURI).href;
const USE_REAL_URLS = location.protocol !== 'file:';

function projectHref(slug) {
  return USE_REAL_URLS ? `projects/${encodeURIComponent(slug)}/` : `#/project/${slug}`;
}

function getRoute() {
  if (PAGE_PROJECT) return { type: 'project', slug: PAGE_PROJECT };
  const h = window.location.hash || '#';
  if (h.startsWith('#/project/')) return { type: 'project', slug: decodeURIComponent(h.replace('#/project/', '')) };
  return { type: 'home' };
}

/* True when the visitor arrived at this project page from the homepage,
   so "Back" can be a real history step (which restores their scroll
   position) instead of a fresh load of the top of the page. */
function cameFromHome() {
  try {
    const ref = new URL(document.referrer);
    const root = new URL(ROOT_URL);
    return ref.origin === root.origin &&
           ref.pathname.replace(/index\.html$/, '') === root.pathname;
  } catch { return false; }
}
...
function buildCard(slug, p, i, wide) {
  return `
    <a class="proj-a${wide ? ' proj-a--wide' : ''} rv${i % 2 ? ' d1' : ''}"
       href="${projectHref(slug)}" data-slug="${slug}" aria-label="${p.title} — ${p.sub}">
      ${buildCardThumb(p)}
      <div class="proj-body">
        <div class="proj-top">
...
function renderProject(slug) {
  ...
  if (!p) {
    if (PAGE_PROJECT) window.location.replace(ROOT_URL);
  ...
  const linksHTML = p.links?.length
    ? `<div class="pd-links">
         ${p.links.map(l =>
           `<a class="pd-link" href="${l.url}" target="_blank" rel="noopener noreferrer" data-track="${slug}: ${l.label}">${l.label} <span aria-hidden="true">↗</span></a>`
         ).join('')}
       </div>`
    : '';
  ...
  const nextHTML = (PROJECT_SLUGS.length > 1 && next)
    ? `<div class="pd-next">
         <a class="pd-next-a" href="${projectHref(nextSlug)}">
           <div>
  ...
  document.getElementById('page-project').innerHTML = `
    <div class="pd-hero">
      <a href="${PAGE_PROJECT ? ROOT_URL + '#work' : '#'}" class="pd-back" id="pd-back">← Back to work</a>
  ...
  /* On a real project page, step back through history when the visitor
     came from the homepage (the browser restores their scroll position);
     otherwise the link's own href takes them to the work section. On a
     legacy hash route, just clear the hash; scroll is restored in route(). */
  document.getElementById('pd-back').addEventListener('click', e => {
    if (PAGE_PROJECT) {
      if (cameFromHome() && history.length > 1) { e.preventDefault(); history.back(); }
      return;
    }
    e.preventDefault();
...
function scrollToSection(id) {
  if (PAGE_PROJECT) {
    window.location.href = ROOT_URL + '#' + id;
  } else if (getRoute().type !== 'home') {
    window.location.hash = '#';
...
document.getElementById('n-home').addEventListener('click', e => {
  e.preventDefault();
  closeMobileMenu();
  if (PAGE_PROJECT) window.location.href = ROOT_URL;
  else if (getRoute().type !== 'home') window.location.hash = '#';
...
/* ══════════════════════════════════════════
   SKIP LINK
   Handled in script rather than by the plain #main fragment: project
   pages carry a <base href="../../">, and under a <base> a fragment-only
   link resolves to the homepage instead of this page.
══════════════════════════════════════════ */
...
function initSkipLink() {
  ...
  link.addEventListener('click', e => {
    e.preventDefault();
    const target = getRoute().type === 'project'
      ? document.getElementById('page-project')
...
/* ══════════════════════════════════════════
   ...
   Loads only when SITE.goatcounter holds a code, so nothing is sent until
   you turn it on in content.js. GoatCounter sets no cookies and needs no
   consent banner. Project pages are real URLs, so each one shows up as its
   own page in the dashboard; old #/project/ links are reported under the
   same projects/<slug>/ path so the numbers line up. Clicks on anything
   with data-track are counted as events.
══════════════════════════════════════════ */

function analyticsPath() {
  const r = getRoute();
  if (r.type === 'project' && !PAGE_PROJECT) {
    return new URL(`projects/${encodeURIComponent(r.slug)}/`, ROOT_URL).pathname;
  }
  return null;
}

function initAnalytics() {
  const code = (typeof SITE !== 'undefined' && SITE.goatcounter || '').trim();
  if (!code || location.protocol === 'file:') return;

  /* Label the links worth counting. Contact cells and resume links are in
     the static HTML; project links are labelled in renderProject(). */
  document.querySelectorAll('a[href$="resume.pdf"]').forEach(a => { a.dataset.track ||= 'resume'; });
  document.querySelectorAll('.ct-cell[data-site-link]').forEach(a => { a.dataset.track = 'contact: ' + a.dataset.siteLink; });
  document.querySelectorAll('a[href^="mailto:"]').forEach(a => { a.dataset.track ||= 'contact: email'; });
  const copyBtn = document.getElementById('ct-copy');
  if (copyBtn) copyBtn.dataset.track = 'contact: copy email';

  window.goatcounter = { path: p => analyticsPath() || p };
  ...
  /* One delegated listener covers links rendered later, too. */
  document.addEventListener('click', e => {
    const el = e.target.closest('[data-track]');
    if (!el || !window.goatcounter?.count) return;
    window.goatcounter.count({ path: el.dataset.track, title: el.dataset.track, event: true });
  });

  /* Legacy hash navigation doesn't reload the page, so count it by hand. */
  window.addEventListener('hashchange', () => {
    const path = analyticsPath();
    if (path && window.goatcounter?.count) window.goatcounter.count({ path });
  });
}
...
initSkipLink();         // skip link that also works under a <base> tag
...
initAnalytics();        // GoatCounter, only if SITE.goatcounter is set
